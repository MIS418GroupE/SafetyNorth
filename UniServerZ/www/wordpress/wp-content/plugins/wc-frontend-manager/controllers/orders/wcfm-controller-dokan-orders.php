		...
		if(!empty($wcfm_orders_array)) {
			...
			foreach($wcfm_orders_array as $wcfm_orders_single) {
				...
				if( $wcfm_is_allow_order_details = apply_filters( 'wcfm_is_allow_order_details', true ) ) {
					$wcfm_orders_json_arr[$index][] =  '<a href="' . get_wcfm_view_order_url($wcfm_orders_single->order_id, $the_order) . '" class="wcfm_dashboard_item_title">#' . esc_attr( $the_order->get_order_number() ) . '</a> by ' . $username;
				} else {
