			...
			if(!is_wp_error($new_product_id)) {
				...
				if(!$has_error) {
					if( get_post_status( $new_product_id ) == 'publish' ) {
						if(!$has_error) echo '{"status": true, "message": "' . $wcfm_products_manage_messages['product_published'] . '", "redirect": "' . apply_filters( 'wcfm_product_save_publish_redirect', get_permalink( $new_product_id ) ) . '", "id": "' . $new_product_id . '", "title": "' . get_the_title( $new_product_id ) . '"}';	
					} elseif( get_post_status( $new_product_id ) == 'pending' ) {
						if(!$has_error) echo '{"status": true, "message": "' . $wcfm_products_manage_messages['product_saved'] . '", "redirect": "' . apply_filters( 'wcfm_product_save_pending_redirect', get_wcfm_products_url( 'pending' ) ) . '", "id": "' . $new_product_id . '", "title": "' . get_the_title( $new_product_id ) . '"}';
					} else {
						if(!$has_error) echo '{"status": true, "message": "' . $wcfm_products_manage_messages['product_saved'] . '", "redirect": "' . apply_filters( 'wcfm_product_save_draft_redirect', get_wcfm_edit_product_url( $new_product_id ) ) . '", "id": "' . $new_product_id . '"}';
					}
				}
				die;
			}
