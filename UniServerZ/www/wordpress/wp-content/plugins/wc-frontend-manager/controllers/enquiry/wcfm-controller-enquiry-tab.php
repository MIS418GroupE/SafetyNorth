		  ...
		  $headers[] = 'Cc: ' . $customer_email;
			...
			if( wcfm_is_marketplace() ) {
				if( $vendor_id ) {
					...
					// Direct message
					$wcfm_messages = sprintf( __( 'You have received an Enquiry for <b>%s</b>', 'wc-frontend-manager' ), '<a class="wcfm_dashboard_item_title" href="' . get_wcfm_enquiry_url() . '">' . get_the_title( $product_id ) . '</a>' );
