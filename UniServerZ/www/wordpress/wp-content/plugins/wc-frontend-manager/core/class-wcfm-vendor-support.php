...
class WCFM_Vendor_Support {
	...
	public function __construct() {
		...
		if ( !is_admin() || defined('DOING_AJAX') ) {
			...
			// Vendor order notification on WCFM Message
  ...
  /**
   * New Order notification on WCFM Message board - 3.0.6
   */
  function wcfm_message_on_new_order( $order_id ) {
  	global $WCFM, $wpdb;
  	
  	if( !$WCFM->is_marketplace ) return;
  	if( is_admin() ) return;
    ...
    foreach ( $items as $item_id => $item ) {
    	...
    	$message_to = $this->wcfm_get_vendor_id_from_product( $product_id );
    	
    	if( $message_to ) {
    		$wcfm_messages = sprintf( __( 'You have received an Order <b>#%s</b> for <b>%s</b>', 'wc-frontend-manager' ), '<a class="wcfm_dashboard_item_title" href="' . get_wcfm_view_order_url($order_id) . '">' . $order_id . '</a>', get_the_title( $product_id ) );
