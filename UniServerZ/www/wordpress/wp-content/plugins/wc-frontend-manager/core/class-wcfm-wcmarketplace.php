  	...
  	$line_total = $item->get_total();
		...
		?>
				...
				<div class="view">
				  ...
				  if( $order_line_due[0]->commission_id ) {
				    if( $admin_fee_mode ) {
				    	...
				    	echo wc_price( $order_line_due[0]->line_total );
				    }
			...
			<td class="line_cost shipping_tax_cost no_ipad no_mob">
				<div class="view"><?php echo ( $order_line_due[0]->shipping_tax_amount == 'NAN' ) ? wc_price( 0 ) : wc_price( $order_line_due[0]->shipping_tax_amount ); ?></div>
			</td>
			<?php } ?>
		<?php
		} else {
		...
		?>
		<td class="line_cost total_cost no_ipad no_mob"><?php echo wc_price( $line_total ); ?></td>
		<?php
  }
    ...
    if( $order_due[0]->commission_id ) {
		...
		<?php if( !$admin_fee_mode ) { ?>
			<tr>
				...
				<td class="total">
					<div class="view"><?php if( $order_due[0]->commission_id ) { echo wc_price( $order_due[0]->line_total ); } else { _e( 'N/A', 'wc-frontend-manager' ); } ?></div>
				</td>
			</tr>
		<?php } ?>
		<?php if ( $WCMp->vendor_caps->vendor_payment_settings('give_shipping') ) { ?>
		...
		<tr>
			...
			<td class="total">
				<div class="view"><?php echo ( $order_due[0]->shipping_tax_amount == 'NAN' ) ? wc_price( 0 ) : wc_price( $order_due[0]->shipping_tax_amount ); ?></div>
			</td>
		</tr>
		<?php } ?>
		<?php
		if( $admin_fee_mode ) {
			...
			<tr>
				...
				<td class="total">
					<div class="view"><?php if( $order_due[0]->commission_id ) { echo wc_price( $total ); } else { _e( 'N/A', 'wc-frontend-manager' ); } ?></div>
				</td>
			</tr>
		<?php
		}
		...
		<tr>
			...
			<td class="total">
				<div class="view">
					<?php 
					echo wc_price( $gross_sale_order ); 
					?>
				</div>
			</td>
		</tr>
